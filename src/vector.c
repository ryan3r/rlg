...
void vector_add(vector_t *vector, void *value) {
    ...
    if (vector->length > vector->allocLength) {
        ...
        vector->begin =
            realloc(vector->begin, vector->allocLength * vector->elementSize);
    }

    memcpy(vector->begin + (vector->length - 1) * vector->elementSize, value,
           vector->elementSize);
}

// remove the last value from a vector
void vector_remove_last(vector_t *vector) { --vector->length; }

// release the memory for a vector
void vector_destroy(vector_t *vector) { free(vector->begin); }
