...
static uint32_t in_room(dungeon_t *d, int16_t y, int16_t x) {
    ...
    for (i = 0; i < d->rooms.length; i++) {
        if ((x >= vector_get(&d->rooms, room_t, i).position[dim_x]) &&
            (x < (vector_get(&d->rooms, room_t, i).position[dim_x] +
    ...
    while ((p = heap_remove_min(&h))) {
        ...
        if ((path[p->pos[dim_y] - 1][p->pos[dim_x]].hn) &&
            ...
            heap_decrease_key_no_replace(
                &h, path[p->pos[dim_y] - 1][p->pos[dim_x]].hn);
        }
        if ((path[p->pos[dim_y]][p->pos[dim_x] - 1].hn) &&
            ...
            heap_decrease_key_no_replace(
                &h, path[p->pos[dim_y]][p->pos[dim_x] - 1].hn);
        }
        if ((path[p->pos[dim_y]][p->pos[dim_x] + 1].hn) &&
            (path[p->pos[dim_y]][p->pos[dim_x] + 1].cost >
             p->cost + hardnesspair(p->pos))) {
...
#define hardnesspair_inv(p)                         \
    (is_open_space(d, p[dim_y], p[dim_x])           \
         ? 127                                      \
         : (adjacent_to_room(d, p[dim_y], p[dim_x]) \
                ? 191                               \
                : (255 - hardnesspair(p))))
...
static int create_cycle(dungeon_t *d) {
    ...
    for (i = max = 0; i < d->rooms.length - 1; i++) {
        for (j = i + 1; j < d->rooms.length; j++) {
            tmp = (((vector_get(&d->rooms, room_t, i).position[dim_x] -
                     vector_get(&d->rooms, room_t, j).position[dim_x]) *
                    (vector_get(&d->rooms, room_t, i).position[dim_x] -
                     vector_get(&d->rooms, room_t, j).position[dim_x])) +
                   ((vector_get(&d->rooms, room_t, i).position[dim_y] -
                     vector_get(&d->rooms, room_t, j).position[dim_y]) *
                    (vector_get(&d->rooms, room_t, i).position[dim_y] -
                     vector_get(&d->rooms, room_t, j).position[dim_y])));
            if (tmp > max) {
                max = tmp;
    ...
    e1[dim_y] = rand_range(vector_get(&d->rooms, room_t, p).position[dim_y],
                           (vector_get(&d->rooms, room_t, p).position[dim_y] +
                            vector_get(&d->rooms, room_t, p).size[dim_y] - 1));
    e1[dim_x] = rand_range(vector_get(&d->rooms, room_t, p).position[dim_x],
                           (vector_get(&d->rooms, room_t, p).position[dim_x] +
                            vector_get(&d->rooms, room_t, p).size[dim_x] - 1));
    e2[dim_y] = rand_range(vector_get(&d->rooms, room_t, q).position[dim_y],
                           (vector_get(&d->rooms, room_t, q).position[dim_y] +
                            vector_get(&d->rooms, room_t, q).size[dim_y] - 1));
    e2[dim_x] = rand_range(vector_get(&d->rooms, room_t, q).position[dim_x],
                           (vector_get(&d->rooms, room_t, q).position[dim_x] +
                            vector_get(&d->rooms, room_t, q).size[dim_x] - 1));
...
static int connect_rooms(dungeon_t *d) {
    ...
    for (i = 1; i < d->rooms.length; i++) {
        connect_two_rooms(d, vector_get_ptr(&d->rooms, room_t, i) - 1,
                          vector_get_ptr(&d->rooms, room_t, i));
    }
            ...
            for (s = t = p = 0; p < 5; p++) {
                for (q = 0; q < 5; q++) {
                    if (y + (p - 2) >= 0 && y + (p - 2) < DUNGEON_Y &&
                        ...
                        s += gaussian[p][q];
                        t +=
                            hardness[y + (p - 2)][x + (q - 2)] * gaussian[p][q];
                    }
                }
            }
            d->hardness[y][x] = t / s;
                ...
                for (p[dim_x] = r->position[dim_x] - 1;
                    ...
                    if (mappair(p) >= ter_floor) {
                        ...
                        empty_dungeon(d);
                               ...
                               (p[dim_y] !=
                                r->position[dim_y] + r->size[dim_y]) &&
                               (p[dim_x] != r->position[dim_x] - 1) &&
                               (p[dim_x] !=
                                r->position[dim_x] + r->size[dim_x])) {
...
static int make_rooms(dungeon_t *d) {
    ...
    for (i = 0; i < d->rooms.length; i++) {
        ...
        while (rand_under(3, 4) &&
               vector_get(&d->rooms, room_t, i).size[dim_x] < ROOM_MAX_X) {
            vector_get(&d->rooms, room_t, i).size[dim_x]++;
        }
        while (rand_under(3, 4) &&
               vector_get(&d->rooms, room_t, i).size[dim_y] < ROOM_MAX_Y) {
            vector_get(&d->rooms, room_t, i).size[dim_y]++;
        }
    }
...
static void init_terrain_map(dungeon_t *dungeon) {
    ...
    vector_for(room_t, room, &dungeon->rooms) {
        for (uint32_t y = room->position[dim_y],
                      end = room->position[dim_y] + room->size[dim_y];
             y < end; ++y) {
            for (uint32_t x = room->position[dim_x],
                          end_x = room->position[dim_x] + room->size[dim_x];
                 x < end_x; ++x) {
...
int save_dungeon(dungeon_t *dungeon, char *file_name) {
    ...
    *((uint32_t *)(header + 16)) = file_size;
    ...
    if (fwrite(dungeon->hardness, sizeof(dungeon->hardness), 1, file) < 1)
        return -1;
...
int load_dungeon(dungeon_t *dungeon, char *file_name) {
    ...
    if (fread(dungeon->hardness, sizeof(dungeon->hardness), 1, file) < 1)
        return -1;
...
static void _calc_travel_costs(dungeon_t *d, bool can_tunnel) {
    ...
    if (GET_PATH == NULL) {
        *(can_tunnel ? &d->paths_tunneling : &d->paths) = d2_malloc(DUNGEON_Y, DUNGEON_X, corridor_path_t);
        ...
        for (y = 0; y < DUNGEON_Y; y++) {
            for (x = 0; x < DUNGEON_X; x++) {
                GET_PATH[y][x].pos[dim_y] = y;
                GET_PATH[y][x].pos[dim_x] = x;
            }
        }
    }
