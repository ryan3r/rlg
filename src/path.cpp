...
static int32_t dist_cmp(const void *key, const void *with) {
  return ((int32_t) dungeon->pc_distance[((path_t *) key)->pos.y]
                                        [((path_t *) key)->pos.x] -
          (int32_t) dungeon->pc_distance[((path_t *) with)->pos.y]
                                        [((path_t *) with)->pos.x]);
}

static int32_t tunnel_cmp(const void *key, const void *with) {
  return ((int32_t) dungeon->pc_tunnel[((path_t *) key)->pos.y]
                                      [((path_t *) key)->pos.x] -
          (int32_t) dungeon->pc_tunnel[((path_t *) with)->pos.y]
                                      [((path_t *) with)->pos.x]);
}
...
void dijkstra(dungeon_t *d)
{
  ...
  d->pc_distance[pc_t::pc->position.y][pc_t::pc->position.x] = 0;
  ...
  while ((c = (path_t*) heap_remove_min(&h))) {
    c->hn = NULL;
    if ((p[c->pos.y - 1][c->pos.x - 1].hn) &&
        (d->pc_distance[c->pos.y - 1][c->pos.x - 1] >
         d->pc_distance[c->pos.y][c->pos.x] + 1)) {
      d->pc_distance[c->pos.y - 1][c->pos.x - 1] =
        d->pc_distance[c->pos.y][c->pos.x] + 1;
      heap_decrease_key_no_replace(&h,
                                   p[c->pos.y - 1][c->pos.x - 1].hn);
    }
    if ((p[c->pos.y - 1][c->pos.x    ].hn) &&
        (d->pc_distance[c->pos.y - 1][c->pos.x    ] >
         d->pc_distance[c->pos.y][c->pos.x] + 1)) {
      d->pc_distance[c->pos.y - 1][c->pos.x    ] =
        d->pc_distance[c->pos.y][c->pos.x] + 1;
      heap_decrease_key_no_replace(&h,
                                   p[c->pos.y - 1][c->pos.x    ].hn);
    }
    if ((p[c->pos.y - 1][c->pos.x + 1].hn) &&
        (d->pc_distance[c->pos.y - 1][c->pos.x + 1] >
         d->pc_distance[c->pos.y][c->pos.x] + 1)) {
    ...
    if ((p[c->pos.y    ][c->pos.x - 1].hn) &&
        (d->pc_distance[c->pos.y    ][c->pos.x - 1] >
         d->pc_distance[c->pos.y][c->pos.x] + 1)) {
      d->pc_distance[c->pos.y    ][c->pos.x - 1] =
        d->pc_distance[c->pos.y][c->pos.x] + 1;
      heap_decrease_key_no_replace(&h,
                                   p[c->pos.y    ][c->pos.x - 1].hn);
    }
    if ((p[c->pos.y    ][c->pos.x + 1].hn) &&
        (d->pc_distance[c->pos.y    ][c->pos.x + 1] >
         d->pc_distance[c->pos.y][c->pos.x] + 1)) {
...
void dijkstra_tunnel(dungeon_t *d)
{
  ...
  d->pc_tunnel[pc_t::pc->position.y][pc_t::pc->position.x] = 0;
  ...
  while ((c = (path_t*) heap_remove_min(&h))) {
    ...
    c->hn = NULL;
    if ((p[c->pos.y - 1][c->pos.x - 1].hn) &&
        (d->pc_tunnel[c->pos.y - 1][c->pos.x - 1] >
         d->pc_tunnel[c->pos.y][c->pos.x] +
         tunnel_movement_cost(c->pos.x, c->pos.y))) {
      d->pc_tunnel[c->pos.y - 1][c->pos.x - 1] =
        (d->pc_tunnel[c->pos.y][c->pos.x] +
         tunnel_movement_cost(c->pos.x, c->pos.y));
      heap_decrease_key_no_replace(&h,
