...
#define vector_get(vector, type, index) (*vector_get_ptr(vector, type, index))

// get a value in a vector
#define vector_get_ptr(vector, type, index) (((type *)(vector)->begin) + index)
...
#define vector_for(type, var, vector)                                        \
    for (type *var = (vector)->begin, *__end_##var = var + (vector)->length; \
         var != __end_##var; ++var)
